...
void Dispatcher::loop() {
  ...
  if (!is_recv && _ms->getMillis() - radio_nonrx_start > 8000) {   // radio has not been in Rx mode for 8 seconds!
    ...
    logFaultEvent(ERR_EVENT_STARTRX_TIMEOUT);
  }
  ...
  // check inbound (delayed) queue
  {
    ...
    if (pkt) {
      ...
      // beebo: rx_wait_relay -- how much this delayed relay's actual dequeue
    ...
    if (len > 0) {
      ...
      if (pkt == NULL) {
        ...
        if (tryParsePacket(pkt, raw, len)) {
          ...
          logRxDisposition(pkt, RX_DISP_PARSE_ERR);   // freed before disposal; error counter only
...
#if defined(RX_DISPOSITION) && defined(BEEBO_CPU_ACCOUNTING)
        pkt->_rx_scheduled_for = scheduled_for;  // beebo: rx_wait_relay, see Packet.h
#endif
...
void Dispatcher::processRecvPacket(Packet* pkt) {
  ...
  if (action == ACTION_RELEASE) {
    onPacketDisposed(pkt);   // disposition is now fully resolved; commit its ring record
    _mgr->free(pkt);
  } else if (action == ACTION_MANUAL_HOLD) {
    onPacketDisposed(pkt);
    ...
    // beebo: queueOutbound() is checked (and, on failure, corrected via
    // logRxDisposition(RX_DISP_QUEUE_FULL)) BEFORE onPacketDisposed() commits
    // this packet's ring record -- onRecvPacket() already logged
    // RX_DISP_FORWARDED as a decision, but that decision can still silently
    // fail to actually enqueue (send_queue full); without this, the
    // committed record would claim FORWARDED regardless of what really
    // happened. queueOutbound() frees `pkt` itself on failure, but reads
    // pkt's own fields immediately after are still safe here: this is all
    // synchronous, single-threaded code, and nothing else touches the
    // packet pool between the free() and the read.
    if (!_mgr->queueOutbound(pkt, priority, futureMillis(_delay))) {
      logRxDisposition(pkt, RX_DISP_QUEUE_FULL);
      logTxQueueFull(true);   // is_relay=true -- richer trace already on this packet's own MON_RX record
    }
    onPacketDisposed(pkt);   // disposition is now fully resolved; commit its ring record
  }
}
...
void Dispatcher::checkSend() {
  ...
  if (_radio->isReceiving()) {
    ...
    if (_ms->getMillis() - cad_busy_start > getCADFailMaxDuration()) {
      ...
      logFaultEvent(ERR_EVENT_CAD_TIMEOUT);
...
void Dispatcher::sendPacket(Packet* packet, uint8_t priority, uint32_t delay_millis) {
  if (!Packet::isValidPathLen(packet->path_len) || packet->payload_len > MAX_PACKET_PAYLOAD) {
    ...
    if (!_mgr->queueOutbound(packet, priority, futureMillis(delay_millis))) {
      logTxQueueFull(false);   // is_relay=false -- self-originated send/ACK reply, no other trace
    }
  }
}
