...
public:
  ...
  // ignores it, no override needed. pkt_airtime_millis is this packet's own
  // estimated on-air time (Dispatcher::getEstAirtimeFor(packet->getRawLength()),
  // same value BaseChatMesh computes before calcDirectTimeoutMillisFor() for
  // the ack side) -- lets an implementation size its own echo-wait window
...
class Mesh : public Dispatcher {
  ...
  void handlePoke(Packet* pkt, uint8_t subtype);
...
protected:
  ...
  */
  ...
  /**
   * \brief  A poke reply has been received (this node originated the request).
   * \param  tag          the tag echoed back from the original request
   * \param  rssi         receiver's measured RSSI (dBm), or INT16_MIN if unavailable (MSG_INCLUDE_RSSI not built)
   * \param  snr          receiver's measured SNR*4
   * \param  noise_floor  receiver's measured noise floor (dBm)
  */
  virtual void onPokeReply(Packet* packet, uint32_t tag, int16_t rssi, int16_t snr, int16_t noise_floor) { }
...
public:
  ...
  Packet* createPoke(uint8_t subtype, const uint8_t* dest_hash, const uint8_t* secret, const uint8_t* data, size_t data_len);
