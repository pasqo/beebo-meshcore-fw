...
// beebo: 'poke' sub-protocol, layered on PAYLOAD_TYPE_RAW_CUSTOM (payload[0] discriminator).
// A direct-route round trip that returns the receiver's atomically-measured RSSI/SNR/noise-floor
// for diagnosing link quality, independent of TRACE (which only returns SNR). See Mesh::createPoke().
#define POKE_SUBTYPE_REQUEST      0x01
#define POKE_SUBTYPE_REPLY        0x02
...
public:
  ...
  // beebo: distilled monitor-ring fields, staged onto this packet at capture
  // (onPacketCaptured) and accumulated by logRxDisposition() as its endpoint/
  // routing verdict resolves. Committed as one finished ring record once
  // disposition is fully known (onPacketDisposed), so this survives untouched
  ...
  // beebo: RouteRecord's rx_wait_relay (kbase/CPU_UTILIZATION.md) -- the
  ...
  /**
   * \brief  SHA256(payload)[0:4] LE -- matches MonRing's TxRecord/RxRecord
   *         pkt_hash convention (payload only, no type-byte prefix), NOT
   *         calculatePacketHash() above. See Packet.cpp for why these are
   *         deliberately separate hash spaces.
   */
  uint32_t calculateMonRingHash() const;
