...
public:
  ...
  // beebo: battery ADC sample resolution, bits. 0 = board doesn't support
  // configuring this (default no-op board). setAdcResolution(0) means
  // "use the board's built-in default".
  virtual bool setAdcResolution(uint8_t bits) { return false; };
  virtual uint8_t getAdcResolution() const { return 0; }
