...
// This header defines two entirely different MultiSerialInterface classes,
// gated by MULTI_TRANSPORT (a beebo-only build flag, never defined by any
// upstream env): beebo's own session-arbitrated aggregator below when
// building multi_role, and upstream's own simpler broadcast-to-all-
// interfaces aggregator (used by examples/companion_radio's other board
// targets) otherwise -- each build target's own #include of this file
// resolves to whichever class its call sites actually expect, so neither
// side's example code needs to change.
...
#else  // !MULTI_TRANSPORT -- upstream's own MultiSerialInterface (verbatim,
       // companion-v1.17.1): a simple broadcast-to-all-registered-interfaces
       // aggregator, used by examples/companion_radio for non-multi_role
       // board targets.
...
public:
  ...
  size_t checkRecvFrame(uint8_t dest[], size_t max_len = MAX_FRAME_SIZE, RecvFrameType* type = nullptr) override {
    ...
    for(auto iface : _interfaces){
      if(iface.instance && iface.instance->isEnabled()){
        size_t frameSize = iface.instance->checkRecvFrame(dest, max_len, type);
        if(frameSize > 0){
          return frameSize;
        }
      }
