...
void RadioLibWrapper::startCW(uint32_t max_ms) {
  _board->onBeforeTransmit();        // key external FEM into TX
  _radio->transmitDirect();          // SET_TX_CONTINUOUS_WAVE at configured freq/power
  _cw_active = true;
  _cw_start = millis();
  _cw_max_ms = max_ms;
}

void RadioLibWrapper::stopCW() {
  _cw_active = false;
  _cw_max_ms = 0;
  _radio->standby();
  _board->onAfterTransmit();         // de-key external FEM
  startRecv();                       // resume normal reception
}

void RadioLibWrapper::loop() {
  if (_cw_active) {                   // CW engaged: radio is owned by the carrier
    // elapsed-time compare (unsigned subtraction) so the safety cap survives
    // the 32-bit millis() wrap — an absolute deadline could vanish or fire early
    if (_cw_max_ms && (uint32_t)(millis() - _cw_start) >= _cw_max_ms) stopCW();
    return;
  }
...
void RadioLibWrapper::startRecv() {
  if (_cw_active) return;            // beebo: don't disturb the CW carrier
  ...
  if (state & STATE_INT_READY) {
    ...
    if (len > 0) {
      ...
      if (err != RADIOLIB_ERR_NONE) {
        ...
        // DLOGL(DLOG_ID_RADIO_RECV_ERROR, "err=%d rssi=%d snr=%d noise=%d", err, rssi, snr, noise_floor);   // beebo: known systematic issue, not worth H severity (RLOGL doesn't exist -- MON_DEBUG can't represent Low severity)
        len = 0;
        n_recv_errors++;
      } else {
...
bool RadioLibWrapper::startSendRaw(const uint8_t* bytes, int len) {
  if (_cw_active) return false;      // beebo: CW carrier owns the radio
